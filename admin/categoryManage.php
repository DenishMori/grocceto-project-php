<script>
    function updateStatus(categorieId) {
        var form = document.getElementById("statusForm" + categorieId);
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                // Update the switch-slider class based on the response
                var switchSlider = document.querySelector("#statusForm" + categorieId + " .switch-slider");
                if (response === "on") {
                    switchSlider.classList.add("active");
                    switchSlider.classList.remove("deactive");
                } else {
                    switchSlider.classList.add("deactive");
        ...
        return false; // prevent the form from submitting and causing a page refresh
    }
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <form action="partials/_categoryManage.php" method="post" enctype="multipart/form-data">
                    <div class="card">
                        <div class="card-header" style="background-color: #63D471;">
                            Create New Category
                        </div>
                        <div class="card-body">
                        ...
                        <div class="card-footer">
                            <div class="row">
                                <div class="col-md-12">
                                    <button type="submit" name="createCategory" class="btn btn-sm btn-secondary col-sm-3 offset-md-4"> Create </button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-md-8 mb-3">
                <div class="card">
                    ...
                    <table class="table table-bordered table-hover mb-0" style="align:center">
                        
                        <thead style="background-color: #63D471;" >
                        <tr>
                        ...
                        <?php 
                            ...
                            while($row = mysqli_fetch_assoc($result)){
                                ...
                                if(isset($_POST['status'.$catId])) {
                                    $status = ($_POST['status'.$catId] == 'on') ? 1 : 0;
                                    $updateSql = "UPDATE `categories` SET `status` = '$status' WHERE `categorieId` = '$catId'";
                                    mysqli_query($conn, $updateSql);
                                }

                                echo '<tr>
                                        <td class="text-center"><b>' .$catId. '</b></td>
                                        <td><img src="/groceryshop/img/card-'.$catId. '.jpg" alt="image for this Category" width="150px" height="150px"></td>
                                        <td>
                                    ...
                                    <form action="" method="POST" id="statusForm'.$catId.'">
                                        <input type="hidden" name="status'.$catId.'" value="'.($status == 1 ? "off" : "on").'">
                                        <button type="submit" onclick="updateStatus(<?=$catId?>) class="toggle-button ">
                                        <div class="switch-rail">
                                        <div class="switch-slider">' .($status == 1 ? "active" : "deactive"). '
                                    </div>   
                                    </div>
                                        </button>
				</form>
        ...
        <form action="partials/_categoryManage.php" method="post" enctype="multipart/form-data">
		    <div class="text-left my-2 row" style="border-bottom: 2px solid #dee2e6;">
				...
				<div class="form-group col-md-4">
					<img src="/groceryshop/img/card-<?php echo $catId; ?>.jpg" id="itemPhoto" name="itemPhoto" alt="Category image" width="100" height="100">
				</div>
			</div>
		</form>
        <form action="partials/_categoryManage.php" method="post">
